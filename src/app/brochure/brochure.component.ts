...
export class BrochureComponent implements OnInit {
  ...
  handleSelection({ index }: any): void {
    ...
    const date = this.brochureVersions[this.selectedKeys[0].split('_')[0]].text;
    const time = this.brochureVersions[this.selectedKeys[0].split('_')[0]].items[this.selectedKeys[0].split('_')[1]].text;

    const thisDate = this.brochureVersions.find(v => v.text === date);
  ...
  createVersion() {
    ...
    const existingInDate = this.brochureVersions.find(v => v.text === nowDate);
    if(existingInDate) {
      const index = this.brochureVersions.findIndex(v => v.text === nowDate);
      let updatedInDate = {...existingInDate};
      updatedInDate.items.unshift({text:nowTime, value: this.currentValue});
      this.brochureVersions[index] = updatedInDate;

    } else {
      this.brochureVersions.unshift({text:nowDate, items: [{ text: nowTime, value: this.currentValue }]});
    }
  ...
  ngOnInit(): void {
    this.brochureService.getVersions().subscribe(brochureVersions => {
     this.brochureVersions = brochureVersions;
     this.initialValue = this.brochureVersions[0].items[0].value;
